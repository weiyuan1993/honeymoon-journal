...
- **Objective:** Extend the Web `花費` page so it remains fast for daily expense entry while also showing the trip's projected total, paid amount, unpaid amount, and complete expense history.
...
The Web `花費` page will provide three sub-tabs: `今日`, `總覽`, and `明細`.
Daily entry remains the default experience, while the other tabs expose the Google Sheet expense plan and the full expense ledger without turning the page into one long view.
...
- **Split `花費` into three sub-tabs** (session-settled: user-directed — chosen over one long page and a two-tab layout: daily entry, trip status, and history are separate tasks). Governs R8-R12.
...
- R8. The `花費` page must contain `今日`, `總覽`, and `明細` sub-tabs and open on `今日` by default.
- R9. `今日` must contain one quick-entry surface, today's totals grouped by original currency, and today's expense entries.
- R10. `總覽` must show `目前預計總花費`, `已實際支出`, `剩餘待付款`, and the major `費用` category breakdowns.
- R11. `明細` must show all `記帳` records and preserve expense search, date, category, and currency filtering as applicable.
- R12. Existing `記帳` editing and deletion behavior must remain available from the history experience.

**Refresh behavior**

- R13. After a new expense is saved, today's currency totals and visible ledger entries must refresh to include it.
...
- R15. `今日` must use the viewer device's local calendar date so the daily view follows the traveler as the device timezone changes.

```mermaid
flowchart TB
  Page["Web 花費 page"] --> Today["今日"]
  Page --> Overview["總覽"]
  Page --> Details["明細"]

  Ledger["Google Sheet 記帳"] --> Today
  Ledger --> Overview
  Ledger --> Details
  Budget["Google Sheet 費用"] --> Overview

  Today --> Quick["Quick entry"]
  Today --> DailyTotals["Totals by original currency"]
  Today --> DailyEntries["Today's entries"]
  ...
  Details --> AllEntries["All entries"]
  Details --> Filters["Search and filters"]
```
...
- F1. Daily expense entry
  ...
  - **Steps:** `今日` opens by default; the user enters an item, amount, currency, and category; the entry is saved; the daily totals and entries refresh.
  - **Covered by:** R8, R9, R13, R15.
- F2. Trip cost review
...
- F3. Historical expense review
  - **Trigger:** The user switches to `明細`.
  - **Steps:** The full ledger appears; the user searches or filters it and can use the existing edit or delete actions.
  - **Covered by:** R11, R12.

### Acceptance Examples

- AE1. **Covers R9, R13, R15.** Given today's ledger contains `EUR 68`, `EUR 9`, and `CHF 18.5`, when the user opens `今日`, then the daily overview shows `EUR 77` and `CHF 18.5` as separate totals.
...
- AE5. **Covers R11, R12.** Given records exist across multiple dates, currencies, and categories, when the user filters `明細`, then only matching entries appear and their existing edit and delete actions remain available.
...
- The device-local date is the correct definition of `今日` during travel.
...
- KTD5. **Keep date and presentation logic pure.** Extract local-date grouping, currency totals, filtering, and overview-label selection into pure frontend helpers. The React page composes these helpers into `今日`, `總覽`, and `明細`, preserving existing edit and delete components. This implements R8-R15 and keeps expensive transforms memoized.
...
flowchart TB
  ...
  ExpensePage --> DailyCalc["Local-day and currency aggregation"]
  DailyCalc --> TodayTab["今日"]
  OverviewAPI --> OverviewTab["總覽"]
  LedgerAPI --> DetailsTab["明細"]
```
...
- **Time behavior:** Today calculations use local calendar keys and refresh on tab re-entry, window focus, and device-local midnight.
...
4. Recompose `ExpensePage` into the three user tasks while preserving ledger mutations.
...
- **Goal:** Derive today's entries, per-currency totals, filtered history, and presentation state from API data.
- **Requirements:** R3-R7, R9-R11, R13-R15; F1-F3; AE1-AE5.
- **Dependencies:** U1.
- **Files:** `src/components/expenseData.ts`, `src/components/expenseData.test.ts`.
- **Approach:** Add pure functions for device-local date keys, daily selection, currency aggregation, filtered history, and overview presentation labels. Combined financial arithmetic remains Worker-owned per KTD1.
...
- **Test scenarios:**
  - Covers AE1. Same-day EUR and CHF entries aggregate separately using the supplied local date.
  - Covers AE2. A complete overview response presents projected, paid, and unpaid without recomputing them.
  - Entries on adjacent local calendar dates appear only in the matching day.
  ...
  - Local timestamps immediately before and after midnight group into different days.
  - Changing the device timezone can move an entry to the newly correct local calendar day without changing its stored timestamp.
  - Invalid timestamps are excluded from today but remain available in history.
- **Verification:** Pure helper outputs match all acceptance examples without relying on component state or browser rendering.

### U4. Recompose the expense page into three tabs

- **Goal:** Deliver the `今日`, `總覽`, and `明細` experiences while preserving quick entry and existing ledger mutations.
- **Requirements:** R7-R15; F1-F3; AE1-AE5.
- **Dependencies:** U2, U3.
...
- **Approach:**
  ...
  2. Keep one quick-entry form on the default `今日` tab.
  3. Show today's original-currency totals and today's entries below the form.
  4. Show TWD projected, paid, and unpaid cards plus category breakdowns on `總覽`.
  5. Move the existing search, filters, grouped history, edit, and delete controls to `明細`.
  6. Refresh visible derived data after save, edit, delete, tab re-entry, window focus, and local midnight.
...
- **Test scenarios:**
  - `今日` is selected on first render and has no duplicate entry form.
  - Saving an expense updates the current-day details and per-currency totals.
  - `總覽` distinguishes projected, paid, and unpaid and identifies unconverted ledger currencies.
  - `明細` preserves search, category, currency, expand, edit, and delete behavior.
  - Ledger success with overview failure leaves `今日` and `明細` usable and shows `總覽` as unavailable rather than zero.
  ...
  - `今日` rolls over after local midnight without a full app reload.
  - Read-only users can inspect all three tabs but cannot invoke mutation controls.
  - Empty, loading, read-only, and fetch-failure states remain understandable on mobile and desktop.
- **Verification:** Browser inspection at mobile and desktop widths confirms the three tasks are distinct, controls do not overflow, and mutations still work for an editor.
...
| Focused frontend helper tests | U3, U4 | Local-day, currency aggregation, filtering, and financial-quality presentation pass. |
...
| Browser behavior | U4 | Local Cloudflare-backed app passes mobile and desktop review for all three expense tabs. |
...
- The Web `花費` page opens on `今日` and exposes `總覽` and `明細` as separate sub-tabs.
- Quick entry, edit, and delete still operate on `記帳`, and daily totals refresh after mutations.
...
- `今日` rolls over at device-local midnight and re-evaluates after focus or timezone changes.
