...
const QUICK_SUGGESTIONS = [
  {
    label: '今日美食',
    question: '推薦今天行程附近的美食，附價位、招牌菜、營業時間與訂位需求。',
  },
  {
    label: '景點重點',
    question: '整理今天行程各景點的歷史與必看重點。',
  },
  {
    label: '明日準備',
    question: '整理今天與明天的準備事項，區分出門前、途中與今晚，並標示待確認資訊。',
  },
];
...
export default function TripSecretaryModal({ isOpen, onClose }: TripSecretaryModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-end justify-center overflow-x-hidden p-3 sm:items-center">
      {/* Backdrop */}
      <div
      ...
      <div className="relative w-full max-w-2xl h-[min(85dvh,760px)] max-h-[calc(100dvh-1.5rem)] border border-white/70 bg-[#fffefa] rounded-3xl shadow-2xl flex flex-col overflow-hidden animate-slide-up">
        {/* Header */}
        <div className="flex shrink-0 flex-wrap items-center justify-between gap-3 border-b border-deep-blue/10 px-5 py-4">
          <div className="flex min-w-0 items-center gap-2">
            ...
            <div className="min-w-0"><h2 className="text-base font-semibold text-deep-blue">旅程秘書</h2><p className="mt-0.5 text-xs text-ink/50">行程、交通與旅途中的大小事</p></div>
          </div>
            ...
            <div className="flex min-w-0 flex-wrap gap-2">
              {QUICK_SUGGESTIONS.map((suggestion) => (
                <button
                  ...
                  className="min-h-9 max-w-full px-3 py-2 text-xs text-deep-blue border border-deep-blue/10 bg-white rounded-xl hover:bg-deep-blue/5 transition-colors"
                >
                  {suggestion.label}
                </button>
              ))}
